/**
 * US Consumer Price Index for All Urban Consumers (CPI-U), annual averages.
 * Index base: 1982–84 = 100.
 *
 * Source: US Bureau of Labor Statistics, series CUUR0000SA0.
 * Last verified against the BLS annual-average table for calendar year 2024.
 *
 * This is the one dataset on the site that goes stale, so the tool that uses
 * it shows LAST_UPDATED prominently rather than implying the numbers are live.
 */

export const CPI_LAST_UPDATED = '2024 annual average (published January 2025)';
...
export const CPI: Record<number, number> = {
	...
	2017: 245.12, 2018: 251.107, 2019: 255.657, 2020: 258.811, 2021: 270.97,
	2022: 292.655, 2023: 304.702, 2024: 313.689
};

export const CPI_YEARS = Object.keys(CPI)
	.map(Number)
	.sort((a, b) => a - b);

export const CPI_FIRST_YEAR = CPI_YEARS[0];
export const CPI_LAST_YEAR = CPI_YEARS[CPI_YEARS.length - 1];

/** Convert an amount from one year's dollars into another's. */
export function adjustForInflation(amount: number, fromYear: number, toYear: number): number {
...
/** Average annual inflation between two years, as a decimal rate. */
export function annualisedInflation(fromYear: number, toYear: number): number {
	const from = CPI[fromYear];
	const to = CPI[toYear];
	const years = toYear - fromYear;
	if (!from || !to || years === 0) return NaN;
